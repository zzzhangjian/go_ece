...
// arModel is a active record design model for table exchange_line operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of exchange_line.
	Table = "exchange_line"
	// Model is the model object of exchange_line.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table exchange_line.
	Columns = struct {
		Id      string // 主键                                                                                     
        Symbol  string // 交易对 btcusdt, ethbtc...（取值参考GET /v1/common/symbols）                              
        Period  string // 返回数据时间粒度，也就是每根蜡烛的时间区间                                               
        Amount  string // 以基础币种计量的交易量 1min, 5min, 15min, 30min, 60min, 4hour, 1day, 1mon, 1week, 1year  
        Count   string // 交易次数                                                                                 
        Open    string // 本阶段开盘价                                                                             
        Close   string // 本阶段收盘价                                                                             
        Low     string // 本阶段最低价                                                                             
