...
// arModel is a active record design model for table tb_chain_transactions operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of tb_chain_transactions.
	Table = "tb_chain_transactions"
	// Model is the model object of tb_chain_transactions.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table tb_chain_transactions.
	Columns = struct {
		Txid              string // 交易ID                   
        BlockNumber       string // 区块高度                 
        TxType            string // 交易类型 omni;erc20;fee  
        Amount            string // 总额                     
        Sendingaddress    string // 发送方                   
        Referenceaddress  string // 接收方                   
        Confirmations     string // 交易的确认数             
        Fee               string // 交易手续费               
        Valid             string // 交易是否有效             
        Positioninblock   string // 交易在区块内的序号       
        CreateAt          string //
	}{
		Txid:             "txid",              
