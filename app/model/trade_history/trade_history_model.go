...
// arModel is a active record design model for table trade_history operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of trade_history.
	Table = "trade_history"
	// Model is the model object of trade_history.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table trade_history.
	Columns = struct {
		Id         string //                                                             
        TradeId    string // 唯一成交ID（NEW）                                           
        Amount     string // 以基础币种为单位的交易量                                    
        Price      string // 以报价币种为单位的成交价格                                  
        Direction  string // 交易方向：“buy” 或 “sell”, “buy” 即买，“sell” 即卖  
        Ts         string // 调整为新加坡时间的时间戳，单位毫秒
	}{
		Id:        "id",         
        TradeId:   "trade_id",   
        Amount:    "amount",     
