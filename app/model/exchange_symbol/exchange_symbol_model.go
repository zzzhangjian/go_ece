...
// arModel is a active record design model for table exchange_symbol operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of exchange_symbol.
	Table = "exchange_symbol"
	// Model is the model object of exchange_symbol.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table exchange_symbol.
	Columns = struct {
		Id               string //                                                                                                                                  
        BaseCurrency     string // 交易对中的基础币种                                                                                                               
        QuoteCurrency    string // 交易对中的报价币种                                                                                                               
        PricePrecision   string // 交易对报价的精度（小数点后位数）                                                                                                 
        AmountPrecision  string // 交易对基础币种计数精度（小数点后位数）                                                                                           
        ...
        MinOrderAmt      string // 交易对最小下单量 (下单量指当订单类型为限价单或sell-market时，下单接口传的'amount')                                               
        MaxOrderAmt      string // 交易对最大下单量                                                                                                                 
        MinOrderValue    string // 最小下单金额 （下单金额指当订单类型为限价单时，下单接口传入的(amount * price)。当订单类型为buy-market时，下单接口传的'amount'）  
        LeverageRatio    string // 交易对杠杆最大倍数                                                                                                               
        Flag             string // 标签位，用于推荐，排序等,默认为0，1表示推荐，                                                                                    
        Sorted           string // 排序                                                                                                                             
        Enable           string // 状态，1：启用，0：禁止
	}{
		Id:              "id",                
        ...
        SymbolPartition: "symbol_partition",  
        Symbol:          "symbol",            
        State:           "state",             
