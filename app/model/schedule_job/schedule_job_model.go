...
// arModel is a active record design model for table schedule_job operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of schedule_job.
	Table = "schedule_job"
	// Model is the model object of schedule_job.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table schedule_job.
	Columns = struct {
		JobId           string // 任务id                      
        BeanName        string // spring bean名称             
        Params          string // 参数                        
        CronExpression  string // cron表达式                  
        Status          string // 任务状态  0：正常  1：暂停  
        Remark          string // 备注                        
