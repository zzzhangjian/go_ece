...
// arModel is a active record design model for table tb_user_relation operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of tb_user_relation.
	Table = "tb_user_relation"
	// Model is the model object of tb_user_relation.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table tb_user_relation.
	Columns = struct {
		Id            string // 主键           
        UserId        string // 用户ID         
        UserParentId  string // 上级用户ID     
        LevelRight    string // 用户层数       
        LevelLeft     string // 用户层数       
        Area          string // 0:左区 1:右区  
        SubLeft       string // 左区业绩       
        SubRight      string // 右区业绩       
        Total         string // 业绩           
        TotalAbs      string // 量碰业绩       
        CreateTime    string //
	}{
		Id:           "id",              
        UserId:       "user_id",         
