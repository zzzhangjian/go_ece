...
// arModel is a active record design model for table exchange_currency_copy operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of exchange_currency_copy.
	Table = "exchange_currency_copy"
	// Model is the model object of exchange_currency_copy.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table exchange_currency_copy.
	Columns = struct {
		Id                       string //                                                                                                
        Currency                 string // 币种                                                                                           
        InstStatus               string // 币种状态normal,delisted                                                                        
        Chain                    string // 链名称                                                                                         
        BaseChain                string // 底层链名称                                                                                     
        BaseChainProtocol        string // 底层链协议                                                                                     
        IsDynamic                string // 否动态手续费（仅对固定类型有效，withdrawFeeType=fixed）	true,false                              
        NumOfConfirmations       string // 安全上账所需确认次数（达到确认次数后允许提币）                                                 
        NumOfFastConfirmations   string // 快速上账所需确认次数（达到确认次数后允许交易但不允许提币）                                     
        MinDepositAmt            string // 单次最小充币金额                                                                               
        DepositStatus            string // 充币状态 allowed,prohibited                                                                    
        MinWithdrawAmt           string // 单次最小提币金额                                                                               
        MaxWithdrawAmt           string // 单次最大提币金额                                                                               
        WithdrawQuotaPerDay      string // 当日提币额度（新加坡时区）                                                                     
        WithdrawQuotaPerYear     string // 当年提币额度                                                                                   
        WithdrawQuotaTotal       string // 总提币额度                                                                                     
        WithdrawPrecision        string // 提币精度                                                                                       
        WithdrawFeeType          string // 提币手续费类型（特定币种在特定链上的提币手续费类型唯一）	fixed,circulated,ratio                 
        TransactFeeWithdraw      string // 单次提币手续费（仅对固定类型有效，withdrawFeeType=fixed）                                      
        MinTransactFeeWithdraw   string // 最小单次提币手续费（仅对区间类型和有下限的比例类型有效，withdrawFeeType=circulated or ratio）  
        MaxTransactFeeWithdraw   string // 最大单次提币手续费（仅对区间类型和有上限的比例类型有效，withdrawFeeType=circulated or ratio）  
        TransactFeeRateWithdraw  string // 单次提币手续费率（仅对比例类型有效，withdrawFeeType=ratio）                                    
        WithdrawStatus           string // 提币状态	allowed,prohibited
	}{
		Id:                      "id",                          
        Currency:                "currency",                    
        InstStatus:              "inst_status",                 
        Chain:                   "chain",                       
        BaseChain:               "base_chain",                  
        BaseChainProtocol:       "base_chain_protocol",         
        IsDynamic:               "is_dynamic",                  
        NumOfConfirmations:      "num_of_confirmations",        
        NumOfFastConfirmations:  "num_of_fast_confirmations",   
        MinDepositAmt:           "min_deposit_amt",             
        DepositStatus:           "deposit_status",              
        MinWithdrawAmt:          "min_withdraw_amt",            
        MaxWithdrawAmt:          "max_withdraw_amt",            
        WithdrawQuotaPerDay:     "withdraw_quota_per_day",      
        WithdrawQuotaPerYear:    "withdraw_quota_per_year",     
        WithdrawQuotaTotal:      "withdraw_quota_total",        
        WithdrawPrecision:       "withdraw_precision",          
        WithdrawFeeType:         "withdraw_fee_type",           
        TransactFeeWithdraw:     "transact_fee_withdraw",       
        MinTransactFeeWithdraw:  "min_transact_fee_withdraw",   
        MaxTransactFeeWithdraw:  "max_transact_fee_withdraw",   
        TransactFeeRateWithdraw: "transact_fee_rate_withdraw",  
